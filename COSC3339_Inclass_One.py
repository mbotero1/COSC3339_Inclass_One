...
def calculate_shipping_cost(weight, destination):
    ...
    if destination == "US":
        ...
        if weight <= 9:
            ...
            # Over 9 lbs, add $1 per extra lb
            extra_weight = weight - 9
            cost = base_cost + (extra_weight * 1.1)
        ...
        if weight <= 5:
            ...
            # Over 5 lbs, add $6 per extra lb
            extra_weight = weight - 5
            cost = base_cost + (extra_weight * 6.0)
...
def curve_scores(scores):
    ...
    while i < scores.len():
        score = scores[i]
        curved_score = score + random.randint(1, 10)
        if curved_score > 100:
            curved_score = 100
